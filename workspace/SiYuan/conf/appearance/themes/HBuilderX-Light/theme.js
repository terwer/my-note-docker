...
var sidebarRightWidth = null;
var sidebarLeftWidth = null;
var flag98 = 0;
//侧栏隐藏状态；
var clyczt = 0;

/*创建边栏鼠标悬浮展开按钮*/
function SidebarMouseHoverExpandButton() {
    sidebarHoverButton = addinsertCreateElement(getHBuiderXToolbar(), "div", "sidebarHoverButton")
    sidebarHoverButton.setAttribute("title", "开启后左右面板自动关闭，鼠标贴边自动展开。");
    AddEvent(sidebarHoverButton, "mousedown", sidebarHoverButtonImplementEven);
    if (localStorage.getItem("isSidebarHover")) _sidebarHoverButtonImplementEven();
}

function sidebarHoverButtonImplementEven() {
    /**获取区域主体 */
    if (flag98 == 1 && openPanel()) {
        flag98 = 0;
        localStorage.removeItem("isSidebarHover");
        (document.querySelectorAll("[HoverBlock]")).forEach((v) => v.remove());
        var dd = document.querySelector('[aria-label="隐藏停靠栏"]');
        if (clyczt == 0) dd.click();
        localStorage.removeItem("sidebarRightWidth");
        localStorage.removeItem("sidebarLeftWidth");

    } else if (flag98 == 0 && closePanel()) {
        flag98 = 1;
        var dd = document.querySelector('[aria-label="显示停靠栏"]');
        if (dd) {
            dd.click();
            clyczt = 0;
        } else {
        ...
        localStorage.setItem("isSidebarHover", true);
        localStorage.setItem("sidebarRightWidth", sidebarRightWidth);
        localStorage.setItem("sidebarLeftWidth", sidebarLeftWidth);
    }
}

function _sidebarHoverButtonImplementEven() {
    /**获取区域主体 */
    if (closePanel()) {
        flag98 = 1;
        var dd = document.querySelector('[aria-label="显示停靠栏"]');
...
function closePanel() {
    ...
    sidebarLeft = column.firstElementChild;
    sidebarRight = column.lastElementChild;
    var LeftActivation = isLeftActivation();
    var RightActivation = isRightActivation();
    if (LeftActivation || RightActivation) {
        if (LeftActivation && "0px" != sidebarLeft.style.width) {
            closeLeftPanel();
            sidebarHoverButton.style.backgroundImage = "url(/appearance/themes/HBuilderX-Light/src/贴边展开2.png)";
            is = 1;
        }
        if (RightActivation && "0px" != sidebarRight.style.width) {
            closeRightPanel();
            sidebarHoverButton.style.backgroundImage = "url(/appearance/themes/HBuilderX-Light/src/贴边展开2.png)";
            is = 1;
        }
...
function openPanel() {
    ...
    if (LeftActivation || RightActivation) {
        if (LeftActivation && "0px" == sidebarLeft.style.width) {
            openLeftPanel()
            sidebarHoverButton.style.backgroundImage = "url(/appearance/themes/HBuilderX-Light/src/贴边展开1.png)";
            is = 1;
        }
        if (RightActivation && "0px" == sidebarRight.style.width) {
            openRightPanel()
...
function isLeftActivation() {
    var items = diguiTooALL(document.getElementById("dockLeft"), (v) => v.getAttribute("data-hotkeylangid") != null);
    for (let index = 0; index < items.length; index++) {
        if (items[index].classList.contains("dock__item--active")) return true;
    }
    return false;
}

function isRightActivation() {
    var items = diguiTooALL(document.getElementById("dockRight"), (v) => v.getAttribute("data-hotkeylangid") != null);
    for (let index = 0; index < items.length; index++) {
        if (items[index].classList.contains("dock__item--active")) return true;
    }
    return false;
...
function closeLeftPanel() {
    ...
    sidebarLeftWidth = sidebarLeft.style.width;

    sidebarLeft.style.width = "0px";
    sidebarLeft.style.position = "fixed";
    sidebarLeft.style.zIndex = "-11";
    leftHoverBlock_close();
}
function leftHoverBlock_close() {
    var LeftHoverBlock = addinsertCreateElement(getHBuiderXToolbar(), "div");
    LeftHoverBlock.setAttribute("HoverBlock", true);
    LeftHoverBlock.style.position = "fixed";
    LeftHoverBlock.style.height = "90%";
    LeftHoverBlock.style.zIndex = "99";
    //LeftHoverBlock.style.backgroundColor = "red";
    LeftHoverBlock.style.top = "5%";
    LeftHoverBlock.style.width = "24px";
    LeftHoverBlock.style.left = "0px";
    AddEvent(LeftHoverBlock, "mouseover", (e) => {
        e.target.remove();
        openLeftPanel();
    })
}
...
function openLeftPanel() {
    if (localStorage.getItem("sidebarLeftWidth")) {
        sidebarLeft.style.width = localStorage.getItem("sidebarLeftWidth");
    } else {
        sidebarLeft.style.width = "400px";
    }
    sidebarLeft.style.position = "static";
    sidebarLeft.style.zIndex = "2";
    leftHoverBlock_open();
}
function leftHoverBlock_open() {
    var LeftHoverBlock = addinsertCreateElement(getHBuiderXToolbar(), "div");
    LeftHoverBlock.setAttribute("HoverBlock", true);
    LeftHoverBlock.style.position = "fixed";
    //LeftHoverBlock.style.backgroundColor = "red";
    LeftHoverBlock.style.width = "200px";
    LeftHoverBlock.style.left = (parseInt(localStorage.getItem("sidebarLeftWidth")) + 24) + "px";
    LeftHoverBlock.style.top = "5%";
    LeftHoverBlock.style.height = "90%";
    LeftHoverBlock.style.zIndex = "9999";
    AddEvent(LeftHoverBlock, "mouseover", (e) => {
        e.target.remove();
        closeLeftPanel();
    })
...
function closeRightPanel() {
    sidebarRightWidth = sidebarRight.style.width;

    sidebarRight.style.width = "0px";
    sidebarRight.style.position = "fixed";
    sidebarRight.style.zIndex = "-11";
    rightHoverBlock_close();
}
function rightHoverBlock_close() {
    var RightHoverBlock = addinsertCreateElement(getHBuiderXToolbar(), "div");
    RightHoverBlock.setAttribute("HoverBlock", true);
    RightHoverBlock.style.position = "fixed";
    //RightHoverBlock.style.backgroundColor = "red";
    RightHoverBlock.style.width = "24px";
    RightHoverBlock.style.right = "0px";
    RightHoverBlock.style.top = "5%";
    RightHoverBlock.style.height = "90%";
    RightHoverBlock.style.zIndex = "9999";
    AddEvent(RightHoverBlock, "mouseover", (e) => {
        e.target.remove();
        openRightPanel();
    })
}
...
function openRightPanel() {
    if (localStorage.getItem("sidebarRightWidth")) {
        sidebarRight.style.width = localStorage.getItem("sidebarRightWidth");
    } else {
        sidebarRight.style.width = "400px";
    }
    sidebarRight.style.position = "static";
    sidebarRight.style.zIndex = "0";
    rightHoverBlock_open();
}
function rightHoverBlock_open() {
    var RightHoverBlock = addinsertCreateElement(getHBuiderXToolbar(), "div");
    RightHoverBlock.setAttribute("HoverBlock", true);
    RightHoverBlock.style.position = "fixed";
    //RightHoverBlock.style.backgroundColor = "red";
    RightHoverBlock.style.width = "200px";
    RightHoverBlock.style.right = (parseInt(localStorage.getItem("sidebarRightWidth")) + 24) + "px";
    RightHoverBlock.style.top = "5%";
    RightHoverBlock.style.height = "90%";
    RightHoverBlock.style.zIndex = "9999";
    AddEvent(RightHoverBlock, "mouseover", (e) => {
...
function exitWithWarning() {
    maxWidth = document.querySelector("div.layout__center").firstElementChild.firstElementChild.offsetWidth;/**获得编辑区域总宽度 */
...
function quickDropDown() {
    ...
    /**找出下分屏 */
    dowm = updowm[1].firstElementChild.firstElementChild;
    /*console.log(dowm);*/
...
function FocusingOnAmplification() {
...
function huifuNodeDocument() {
    ...
    for (let index = 0; index < dataDocType.length; index++) {
        ...
        if (item == "NodeDocument") {
            continue;
        } else if (item == element.firstElementChild.getAttribute("data-type")) {
...
/**----------------------思源行内备注信息显示弹窗改造。-------------------------------- */
function officialNotesShowRetrofit() {
    AddEvent(document.body, "mouseover", (e) => {
        var target = e.target;

        if (target.getAttribute("data-type") === "inline-memo") {

            var id = setInterval(() => {

                var tooltip = document.getElementById("tooltip");
                if (tooltip == null) return;
                if (tooltip.getAttribute("inlinememo") != null) return;
                clearInterval(id);
                var txtStr = tooltip.innerText;
                tooltip.innerHTML = "";
                tooltip.setAttribute("inlinememo", true);

                var tooltip2 = addinsertCreateElement(tooltip, "div");
                tooltip2.style.borderRadius = "4px";
                tooltip2.style.minWidth = "380px";
                tooltip2.style.maxWidth = "680px";
                tooltip2.style.wordBreak = "break-all";
                tooltip2.style.color = "rgb(109,108,108)";
                tooltip2.style.background = "rgb(255,250,233)";
                tooltip2.style.fontWeight = "lighter";
                tooltip2.style.fontSize = "140%";
                tooltip2.style.boxShadow = "0px 11px 15px -7px rgba(0, 0, 0, 0.2), 0px 24px 38px 3px rgba(0, 0, 0, 0.14), 0px 9px 46px 8px rgba(0, 0, 0, 0.12)";
                tooltip2.style.padding = "0px";
                tooltip2.style.marginLeft = "-10px";
                tooltip2.style.marginTop = "-5px";
                tooltip2.style.marginBottom = "-5px";
                tooltip2.innerHTML = "<div style='font-size:60%;text-align:center; background:rgb(101,168,99); color:rgb(255,255,255); border-top-left-radius:4px; border-top-right-radius:4px;'><span>行内备注</span></div><div style='padding-top:13px;padding-left: 10px; padding-right:8px;padding-bottom: 6px;'>" + txtStr + "</div>";
            }, 0);
...
function Removefirstlineindent() {
    ...
    AddEvent(document.body, "dblclick", (e) => {
        ...
        if ((localStorage.getItem("段落缩进") != "1") && (getComputedStyle(document.getElementsByTagName("head")[0]).backgroundColor != "rgba(0, 0, 0, 0)")) return;
        var element = e.target;
...
function init() {
    ...
    settingBtn.insertAdjacentHTML(
        "afterend",
        '<div id="myHistory" style="position:absolute;z-index:100;top:30em;left:24.4em;width:40em;height:52.8em;background-color: rgb(253,250,239);box-shadow: 0px 11px 15px -7px rgba(0, 0, 0, 0.2), 0px 24px 38px 3px rgba(0, 0, 0, 0.14), 0px 9px 46px 8px rgba(0, 0, 0, 0.12); border:1px solid rgb(238,230,206);visibility:hidden;transform: translate(-50%, -50%);overflow:auto;padding:0px 25px 10px 25px;"><div style="position:sticky;top:0px;padding-top:20px;margin-top:-2px;padding-bottom:15px; background-color:rgb(253,250,239);" class="topBar"><input id="history_input" class="b3-text-field fn__flex-center fn__size200"style="margin-left:5px;" type="text" placeholder="搜索历史记录"  size="30"><button id = "showAllHistory"  class="b3-button b3-button--outline fn__flex-center" style="margin-left:5px;">显示全部</button><button id = "clearHistory"  class="b3-button b3-button--outline fn__flex-center" style="position:absolute;right:10px;">清除历史</button></div><div id ="historyContainer"></div></div>'
    );
    ...
    function showAllHistoryItems(e) {
        ...
        if (
            ...
            tempArr.forEach((value) => {
                ...
                elem_div.style.overflow = "hidden";
                elem_div.style.textOverflow = "ellipsis";
                elem_div.style.whiteSpace = "nowrap";
                elem_div.style.borderBottom = "1px dashed rgba(0, 0, 0, 0.249)";
                elem_div.innerHTML = `<span class="historyTimeStamp" style="color: rgb(95,91,83);margin-left: 10px;margin-right: 1em; overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${item_time}</span>
        <span><svg class="history-icon" style="height:16px;width:16px;vertical-align: middle;">${history_item_icon}</svg></span>
        <span style="color:#3481c5;margin-left:5px;cursor: pointer;overflow:hidden; text-overflow:ellipsis;"data-type="a" data-href="${href}">${item_text}</span>`;
        ...
        if (historyInputArea.value.trim()) {
            ...
            if (
                ...
                tempArr.forEach((value) => {
                    ...
                    elem_div.style.textOverflow = "ellipsis";
                    elem_div.style.whiteSpace = "nowrap";
                    elem_div.innerHTML = `<span class="historyTimeStamp" style="color: rgb(95,91,83);margin-left: 10px;margin-right: 1em; ">${item_time}</span>
            <span><svg class="history-icon" style="height:16px;width:16px;vertical-align: middle;">${history_item_icon}</svg></span>
            <span style="color:#3481c5;margin-left:5px;cursor: pointer;"data-type="a" data-href="${href}" >${item_text}</span>`;
                    fragment.appendChild(elem_div);
                });
                historyContainer.appendChild(fragment);
            }
 ...
 */
function HBuiderXThemeToolbarAddButton(ButtonID, ButtonTitle, NoButtonSvgURL, OffButtonSvgURL, NoClickRunFun, OffClickRunFun, Memory) {
...
setTimeout(() => {
    ...
    if (isPhone()) {
        ...
        officialNotesShowRetrofit();//思源行内备注信息显示弹窗改造。
